// ========== GABUNGAN DATA LEASING ==========
const LEASING_DATA = { ADIRA: LEASING_ADIRA, MUF: LEASING_MUF, BRI: LEASING_BRI };
const ALL_LEASINGS = ['ADIRA', 'MUF', 'BRI', 'SUFI', 'BCA', 'BNI', 'MANDIRI'];
...
const APP = {
  ...
  buildIndexes() {
    ...
    this.db.modelPatterns = [
      ...
      { regex: /(NEW\s*)?XL-?7\s*03.*KURO/i, model: 'XL-7 MC Hybrid Kuro', extract: s => s.replace(/.*(NEW\s*)?XL-?7\s*03\s*(HYBRID\s*)?(KURO\s*)?/i, '') },
      { regex: /(NEW\s*)?XL-?7\s*03.*(?:HYBRID|BETA|ALPHA)/i, model: 'XL-7 MC Hybrid', extract: s => s.replace(/.*(NEW\s*)?XL-?7\s*03\s*(?:HYBRID\s*)?/i, '') },
      { regex: /(NEW\s*)?XL-?7\s*03/i, model: 'XL-7 MC', extract: s => s.replace(/.*(NEW\s*)?XL-?7\s*03\s*/i, '') },
      { regex: /(NEW\s*)?XL-?7\s*04.*(?:HYBRID|BETA|ALPHA)/i, model: 'XL-7 Hybrid', extract: s => s.replace(/.*(NEW\s*)?XL-?7\s*04\s*(?:HYBRID\s*)?/i, '') },
  ...
  parseModelType(raw) {
    ...
    for (let p of this.db.modelPatterns) {
      if (p.regex.test(s)) {
        ...
        if ((p.model === 'XL-7' || p.model === 'XL-7 Hybrid') && !/^NEW\b/i.test(type)) {
          type = 'New ' + type;
        }
